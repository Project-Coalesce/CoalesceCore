...
public interface ISection {
	...
	default Set<String> getKeys(boolean deep) {
		...
		else getConfig().getEntries().stream().filter(e -> e.getPath().startsWith(getCurrentPath()+".")).forEach(e -> {
	...
	default ISection getParent() {
		...
		else return new Section(getCurrentPath().substring(0, getCurrentPath().lastIndexOf(".")), getConfig(), getPlugin());
	}
