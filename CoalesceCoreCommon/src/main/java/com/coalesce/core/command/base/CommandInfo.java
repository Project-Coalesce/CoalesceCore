...
public final class CommandInfo {
    ...
    public CommandInfo(Object container, Method method, CompletionInfo completionInfo, ICoPlugin plugin) {
        ...
        if (method.isAnnotationPresent(Alias.class)) {
            alias = method.getAnnotation(Alias.class);
        }
        if (method.isAnnotationPresent(Sender.class)) {
            sender = method.getAnnotation(Sender.class);
        }
    ...
    public boolean run(CoSender sender, String... args) {
        ...
        if (!senderType) {
            StringBuilder builder = new StringBuilder();
            for (SenderType type : senderTypes)
        ...
        if (registrationType == RegistrationType.ANNOTATION) {
            ...
            commandExecutor.run(new CommandContext(sender, args, plugin));
        }
        return true;
    }
