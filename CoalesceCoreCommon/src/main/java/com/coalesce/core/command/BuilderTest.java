...
public final class BuilderTest {
    ...
    public BuilderTest(ICoPlugin plugin) {
        ...
        ProcessedCommand command = ProcessedCommand.builder(plugin, "test2").permission("core.test1").executor(this::testCommand2).completer(this::testCompletion).minArgs(1).senders(SenderType.PLAYER).usage("/test").description("tests the builder pattern command registration").build();
